...
test_data_folder = os.path.join('data', 'all_runs')
...
def classification_run(folder, f_load, f_cost, ftype='cost'):
    ...
    test_files = [os.path.join(test_data_folder, pair[0]) for pair in pairs]
    train_files = [os.path.join(test_data_folder, pair[1]) for pair in pairs]
    ...
    # costM = np.random.rand(ntest, ntrain)
    if ftype == 'cost':
...
def LoadImg(pth):
    # Load image file
    #
    I = torch.Tensor(imread(pth, as_gray=True))
    return I.reshape(1, 1, I.shape[0], I.shape[1])


def SiamNetScore(itemA, itemB):
    similarity_score = pretrained_model(itemA, itemB)
...
if __name__ == "__main__":
    ...
    for r in range(1, nrun + 1):
        ...
        if len(rs) == 1:
            rs = '0' + rs
        # perror[r - 1] = classification_run(os.path.join(test_data_folder, 'run' + rs), LoadImgAsPoints,
        #                                    ModHausdorffDistance, 'cost')
        perror[r - 1] = classification_run(os.path.join(test_data_folder, 'run' + rs), LoadImg, SiamNetScore, 'score')
